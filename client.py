...
class Client:
    ...
    def perform_key_exchange(self):
            self.sock.sendall(self.private_key.public_key().export_key(format='PEM'))
...
def login():
    ...
    messaging_window = MessagingWindow(host, port, email, password)
...
def update_contacts_list(self):
    ...
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute('SELECT users.name FROM users INNER JOIN contacts ON users.email = contacts.contact_email '
                   'WHERE contacts.user_email = ?', (self.email,))
...
def on_close(self):
    response = messagebox.askokcancel("Confirm", "Are you sure you want to exit?")
    if response:
